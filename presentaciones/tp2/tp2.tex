    ...
    \begin{column}{0.70\textwidth}
      \begin{columns}[T,onlytextwidth]
        \begin{column}{0.5\textwidth}
          \animbox{anim_vicsek_rho2_eta05.png}{$\eta = 0.5\uni{rad}$, $v_a = 0.98$}{\linkpendiente}
        \end{column}
        \begin{column}{0.5\textwidth}
          \animbox{anim_vicsek_rho2_eta4.png}{$\eta = 4\uni{rad}$, $v_a = 0.04$}{\linkpendiente}
        \end{column}
      \end{columns}
    \end{column}
    \begin{column}{0.28\textwidth}
      {\small Vicsek, $\rho = 2$ ($N = 200$) \\[0.35em]
      Vectores $=$ velocidad, color $=$ ángulo. \\[0.35em]
      Fotograma en $t = 1500\uni{s}$; los $v_a$ son de ese instante, no
      promedios. \\[0.35em]
...
\begin{frame}{Vicsek: $v_a(t)$}
  \figuraparam{evolucion_va_vicsek_rho4.png}{%
    ...
    Menos ruido ordena antes y satura más alto.\\[0.35em]
    {\scriptsize Trazos punteados: comienzo del estacionario.}
  }
\end{frame}
...
\begin{frame}{Vicsek: $v_a$ vs $\eta$}
  \figuraparam{vicsek_va_vs_eta.png}{%
    ...
    \textbf{Observación}\\[0.2em]
    De orden total sin ruido a desorden con $\eta$ alto; mayor $\rho$ corre la
    caída hacia ruidos mayores.\\[0.35em]
...
\begin{frame}{Vicsek: $S(t)$}
  \figuraparam{evolucion_s_vicsek_rho2.png}{%
    ...
    \[ \eta \in \{0,\,0.5,\,2\}\uni{rad} \]
    \textbf{Observación}\\[0.2em]
    $S \approx 1$ para los tres ruidos: con $\rho \ge 2$ el grafo de vecinos
    percola y el cluster más grande abarca casi todo.\\[0.35em]
    {\scriptsize Eje vertical acotado a los datos.}
  }
\end{frame}
...
\begin{frame}{Vicsek: $S$ vs $\eta$}
  \figuraparam{vicsek_s_vs_eta.png}{%
    ...
    $\rho \in \left\{\frac{1}{3\pi},\,\frac{1}{2\pi},\,\frac{1}{\pi},\,2,\,4,\,8\right\}$\\[0.45em]
    \textbf{Observación}\\[0.2em]
    Para $\rho \ge 2$, $S \approx 1$ con todo ruido; en las densidades bajas
    cae con $\eta$ y se aplana desde $\eta \approx 4\uni{rad}$.\\[0.35em]
    {\scriptsize Barras: desvío muestral entre seeds.}
  }
\end{frame}
...
\begin{frame}{Votante: animación}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \begin{columns}[T,onlytextwidth]
        \begin{column}{0.5\textwidth}
          \animbox{anim_voter_rho2_eta05.png}{$\eta = 0.5\uni{rad}$, $v_a = 0.55$}{\linkpendiente}
        \end{column}
        \begin{column}{0.5\textwidth}
          \animbox{anim_voter_rho2_eta4.png}{$\eta = 4\uni{rad}$, $v_a = 0.10$}{\linkpendiente}
        \end{column}
      \end{columns}
    \end{column}
    \begin{column}{0.28\textwidth}
      {\small Votante, $\rho = 2$ ($N = 200$) \\[0.35em]
      Misma densidad, mismos ruidos, mismo instante y misma representación que
      Vicsek. \\[0.35em]
      A $\eta = 0.5\uni{rad}$ Vicsek está en $v_a = 0.98$ y el votante ya
...
\begin{frame}{Votante: $v_a(t)$}
  \figuraparam{evolucion_va_voter_rho4.png}{%
    ...
    \[ \eta \in \{0,\,0.5,\,2\}\uni{rad} \]
    \textbf{Observación}\\[0.2em]
    Sin ruido satura en 1; con $0.5\uni{rad}$ fluctúa sin estacionarse; con
    $2\uni{rad}$ queda cerca de cero.\\[0.35em]
...
\begin{frame}{Votante: $v_a$ vs $\eta$}
  \figuraparam{voter_va_vs_eta.png}{%
    ...
    $\rho \in \left\{\frac{1}{3\pi},\,\frac{1}{2\pi},\,\frac{1}{\pi},\,2,\,4,\,8\right\}$\\[0.45em]
    \textbf{Observación}\\[0.2em]
    Todas las densidades pierden el orden dentro del primer radián; a
    diferencia de Vicsek, subir $\rho$ no corre la caída.\\[0.35em]
    {\scriptsize Barras: desvío muestral entre seeds.}
  }
\end{frame}
...
\begin{frame}{Vicsek vs votante: $v_a$ vs $\eta$}
  \figuraparam{comparacion_va_vs_eta_rho4.png}{%
    ...
    ruidos mucho menores: promediar filtra el ruido, copiar a uno solo no.\\[0.35em]
    {\scriptsize Misma forma en $\rho = 2$ y $8$.}
  }
\end{frame}
...
\begin{frame}{Votante: $S(t)$}
  \figuraparam{evolucion_s_voter_rho2.png}{%
    ...
    \[ \eta \in \{0,\,0.5,\,2\}\uni{rad} \]
    \textbf{Observación}\\[0.2em]
    Mismo comportamiento que Vicsek: $S \approx 1$ para los tres ruidos.\\[0.35em]
    {\scriptsize Eje vertical acotado a los datos.}
  }
\end{frame}
...
\begin{frame}{Votante: $S$ vs $\eta$}
  \figuraparam{voter_s_vs_eta.png}{%
    ...
    $\rho \in \left\{\frac{1}{3\pi},\,\frac{1}{2\pi},\,\frac{1}{\pi},\,2,\,4,\,8\right\}$\\[0.45em]
    \textbf{Observación}\\[0.2em]
    Igual que en Vicsek: $\rho \ge 2$ percola y las densidades bajas caen con
    $\eta$. La conectividad depende de la densidad, no de la regla.\\[0.35em]
    {\scriptsize Barras: desvío muestral entre seeds.}
  }
\end{frame}
...
\begin{frame}{Tiempos del CIM}
  \figuraparam[0.58\textwidth]{cim_tp1_vs_tp2.png}{%
    ...
    \textbf{Observación}\\[0.2em]
    Vicsek y votante dan tiempos casi idénticos: el CIM no depende de la regla.
    El TP2, medido dentro de la simulación, queda por encima del microbenchmark
    del TP1.
  }
\end{frame}
