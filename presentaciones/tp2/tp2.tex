...
% Barrido principal de eta: 15 valores discretos (paso 0.25 hasta 1, luego 0.5
% hasta 5.5, mas 2*pi). Lista explicita: en proyeccion la notacion por
% comprension (uniones de conjuntos indexados) no se lee.
\newcommand{\etasweep}{%
  \eta \in \{0;\;0{,}25;\;0{,}5;\;0{,}75;\;1;\;1{,}5;\;2;\;\dots;\;5{,}5;\;6{,}28\}\uni{rad}%
}
% Forma abreviada para el costado de las figuras de Resultados: el detalle de
% los valores ya esta en la diapositiva de Simulaciones.
\newcommand{\etacorto}{$\eta$: 15 valores entre 0 y $6{,}28\uni{rad}$}
\newcommand{\rhocorto}{$\rho$: 6 valores entre $\frac{1}{3\pi}$ y 8}
...
\begin{frame}{Vicsek: $v_a$ vs $S$}
  \figuraparam[0.66\textwidth]{va_vs_s_vicsek.png}{%
    \textbf{Fijos}\\[0.2em]
    Modelo: Vicsek\\
    50 seeds por punto\\[0.45em]
    \textbf{Variables}\\[0.2em]
    Un punto por $\eta$\\
    $\rho$: bajas (izq.), exigidas (der.)
  }
\end{frame}
...
\begin{frame}{Votante: $v_a$ vs $S$}
  \figuraparam[0.66\textwidth]{va_vs_s_voter.png}{%
    \textbf{Fijos}\\[0.2em]
    Modelo: votante\\
    50 seeds por punto\\[0.45em]
    \textbf{Variables}\\[0.2em]
...
\begin{frame}{Vicsek vs votante: $v_a$ vs $S$}
  \figuraparam[0.66\textwidth]{comparacion_va_vs_s.png}{%
    \textbf{Fijos}\\[0.2em]
    50 seeds por punto\\[0.45em]
    \textbf{Variables}\\[0.2em]
    Modelo: Vicsek y votante\\
    Un punto por $\eta$\\
    $\rho$: bajas (izq.), exigidas (der.)
  }
\end{frame}
