...
\begin{frame}{Observables}
  ...
  \vspace{0.6em}
  Valor de cada corrida: $\langle v_a \rangle$, $\langle S \rangle$ $=$ promedio
  temporal para $t \ge T/2$.
  \begin{itemize}
    \item $T = 2000$ pasos ($5000$ si $\rho < 2$)
    \item 50 repeticiones por punto
    \item Curvas: media $\pm$ desvío entre repeticiones
  \end{itemize}
\end{frame}
    ...
    \begin{column}{0.70\textwidth}
      \begin{columns}[T,onlytextwidth]
        \begin{column}{0.5\textwidth}
          \animbox{anim_vicsek_rho2_eta05.png}{$\eta = 0.5$}{\yt{P-lkSdXIhaI}}
        \end{column}
        \begin{column}{0.5\textwidth}
          \animbox{anim_vicsek_rho2_eta4.png}{$\eta = 4$}{\yt{ez4dm6Y6-3w}}
        \end{column}
      \end{columns}
    \end{column}
...
\begin{frame}{Votante: animación}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \begin{columns}[T,onlytextwidth]
        \begin{column}{0.5\textwidth}
          \animbox{anim_voter_rho2_eta05.png}{$\eta = 0.5$}{\yt{tHQbmPHvScs}}
        \end{column}
        \begin{column}{0.5\textwidth}
          \animbox{anim_voter_rho2_eta4.png}{$\eta = 4$}{\yt{UtGcnTB9IRE}}
        \end{column}
      \end{columns}
    \end{column}
    \begin{column}{0.28\textwidth}
...
\begin{frame}{Vicsek vs votante: $S$ vs $\eta$}
  \figuraparam{comparacion_s_vs_eta_rho2.png}{%
    $\rho = 2$ ($N = 200$)\\[0.45em]
    Eje vertical acotado a los datos.
  }
\end{frame}
...
\begin{frame}{Vicsek vs votante: $v_a$ vs $S$}
  \figuraparam{comparacion_va_vs_s.png}{%
    $\rho = \frac{1}{\pi}$ ($N = 32$)
  }
\end{frame}
...
\begin{frame}{Conclusiones}
  \begin{itemize}
    \item Vicsek es mucho más robusto al ruido que el votante: con $\rho = 4$ y
      ...
      ya con $\eta = 0.5\uni{rad}$ el votante cae a $0.28 \pm 0.04$
      mientras Vicsek sigue en $0.99$.
    \item La densidad determina el tamaño del cluster más grande: con $\rho = 2,\,4,\,8$,
      $S \ge 0.96$ para todo ruido, con un mínimo en la transición de cada modelo;
      con $\rho = \frac{1}{\pi}$ cae de $0.98$ a $0.16$ recorriendo $\eta$.
    \item En el plano $v_a$--$S$ ambos modelos recorren la misma curva: la regla
      cambia el $\eta$ al que se llega a cada punto, no la relación entre observables.
    \item El CIM dentro de la simulación tarda lo mismo que en el TP1; Vicsek
      queda un $20\%$ arriba en $N = 200$ y $400$ por las bandadas.
  \end{itemize}
\end{frame}
