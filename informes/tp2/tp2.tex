...
% Anchos de figura. Con 0.70\textwidth para una curva sola, los flotantes se
% acumulaban en paginas de solo figuras, lejos del texto que las cita.
...
pérdida de orden hacia valores mayores de ruido. Para $\rho=2$, $4$ y $8$,
...
tres densidades complementarias menores donde la conectividad deja de ser
trivial.
...
\begin{equation}
    ...
    \label{eq:voter}
\end{equation}
El enunciado exige copiar una sola dirección y no excluye la propia, por lo que
se admitió $k=i$. Cuando no hay vecinos, la Ec.~\eqref{eq:voter} se reduce a
conservar la dirección y sumar ruido.
...
\end{equation}
donde $\Delta t$ es el paso temporal y la operación módulo $L$ implementa las
condiciones periódicas de contorno. Las Ecs.~\eqref{eq:vicsek},
...
temporal: a partir del estado en $t$ se construye la lista de vecinos con el
Cell Index Method (CIM) del TP1, se calculan todas las direcciones nuevas con
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[>=Stealth, font=\small, scale=0.9,
      ...
      \node[caja, right=0.6cm of estado] (cim)
        {Vecinos $< r_c$\\ CIM del TP1\\ {\scriptsize radio $0$, PBC}};
...
tiempo de la llamada al CIM; las animaciones, las figuras y los promedios se
obtienen después, sin volver a simular.
...
\begin{table}[htbp]
    \centering
    \caption{Parámetros del barrido principal, comunes a ambos modelos.}
...
Ec.~\eqref{eq:density} a $N=11$, $16$ y $32$. Se eligieron porque el número
medio de vecinos de una distribución uniforme puede estimarse como
$\rho\pi r_c^2$; los tres valores corresponden aproximadamente a $1/3$, $1/2$
y $1$ vecino, respectivamente. En adelante se las llama densidades
complementarias, y densidades principales a $\rho=2$, $4$ y $8$.
...
Para cada realización $k$ se descartó la primera mitad de la corrida y se
promedió el resto:
\begin{equation}
...
$t\geq T/2$. El corte en $T/2$ se justifica en la
Sección~\ref{sec:resultados} a partir de las evoluciones temporales. Sobre las
$R=50$ realizaciones se calcularon
\begin{align}
...
barras de error, el desvío estándar de la Ec.~\eqref{eq:ensemble-std} entre
realizaciones, no el error estándar de la media. Se dispersa entre
realizaciones y no entre instantes porque los instantes de una misma corrida
están autocorrelacionados, con tiempos de correlación de entre $40$ y $90$
pasos; tratarlos como muestras independientes subestimaría el desvío en un
factor de entre $8$ y $14$. Esta convención rige para todas las figuras de la
Sección~\ref{sec:resultados}: salvo aclaración en contrario, cada punto
promedia las $R=50$ realizaciones, la barra es $\sigma_A$ y las rectas solo
unen puntos consecutivos como guía para el ojo, sin ser un ajuste.
...
\begin{figure}[htbp]
    ...
    instante y esta realización, no promedios de ensamble. Debajo de cada
    panel, el enlace a la animación completa.}
...
calculados con las Ecs.~\eqref{eq:stationary-average} y
\eqref{eq:ensemble-mean}, son $\langle\overline{\va}\rangle=1{,}000$;
...
$0{,}14\,\sigma_A$ en mediana y a lo sumo $0{,}7\,\sigma_A$: el resultado no
...
densidades principales. En $\rho=2$ aparecen fragmentaciones breves, pero el
...
\begin{figure}[htbp]
    ...
    tres densidades principales. Cada panel corresponde a una realización y
...
\begin{figure}[htbp]
    ...
    principales, con el eje horizontal acotado a su propio rango: en una escala
...
corrida sin ruido alcanza el estado absorbente $\va=1$ en $t\simeq320$ s. Para
...
último: el ensamble es estacionario aunque cada realización fluctúe, lo que
respalda el promedio sobre $t\geq T/2$, y el desvío entre realizaciones
conserva esa dispersión. Los promedios estacionarios de las tres curvas son
$\langle\overline{\va}\rangle=1{,}000$; $0{,}28\pm0{,}04$ y
$0{,}079\pm0{,}003$.
...
\begin{figure}[htbp]
    ...
    comienzo del régimen estacionario de la curva de su color; la de
    $\eta=0{,}5$ rad no se marca porque esa realización no alcanza un plateau
    estrecho, aunque el ensamble sí es estacionario.}
...
solo no promedia nada, así que agregar vecinos no aporta resistencia al ruido.
...
\begin{figure}[htbp]
    ...
    las tres densidades principales. Cada panel corresponde a una realización y
...
$0{,}999$. En las densidades complementarias, $S$ disminuye con el ruido y se
aproxima a los mismos valores bajos que en Vicsek.
...
En la Fig.~\ref{fig:va-s-voter}, las densidades principales vuelven a formar una
...
El motor registra el tiempo de cada llamada al CIM, una por paso. Se
corrieron cinco semillas por modelo, de 1000 pasos y con $\eta=0{,}5$ rad,
para $N=200$, $400$ y $800$: son las densidades principales y cubren el rango
de tamaños medido en el TP1 bajo la misma geometría ($L=10$ m, $M=10$ celdas
por lado, $r_c=1$ m, condiciones periódicas y partículas puntuales). De cada
corrida se promediaron las 500 llamadas de la segunda mitad, con el mismo
criterio aplicado a los observables, y luego se promedió entre corridas; la
dispersión es el desvío estándar entre corridas.

Del lado del TP1 se usó el mismo código del CIM con su bench, corrido sobre
50 configuraciones uniformes con semillas distintas y una sola llamada
cronometrada a \texttt{findNeighbors} por configuración, tras calentar la
JVM: así ninguna llamada repite una configuración ya vista, igual que en el
TP2, donde las partículas se mueven entre llamada y llamada. El promedio y el
desvío son sobre las 50 configuraciones. Un tiempo de ejecución solo es
comparable contra otro medido en el mismo equipo y en las mismas condiciones,
así que las tres series se tomaron en la misma máquina (AMD Ryzen 7 5700U,
OpenJDK 21) y en la misma sesión. La Tabla~\ref{tab:cim} y la
Fig.~\ref{fig:cim} las reúnen.

\begin{table}[htbp]
    \centering
    \caption{Tiempo por llamada al CIM en milisegundos, para el bench del TP1
    (50 configuraciones) y para ambos modelos del TP2 ($\eta=0{,}5$ rad,
    cinco semillas, 500 llamadas cronometradas por corrida).}
    \label{tab:cim}
    \begin{tabular}{lccc}
        \toprule
        Serie & $N=200$ & $N=400$ & $N=800$ \\
        \midrule
        TP1 & $0{,}058\pm0{,}009$ & $0{,}18\pm0{,}02$ & $0{,}66\pm0{,}07$ \\
        TP2, Vicsek  & $0{,}07\pm0{,}01$ & $0{,}23\pm0{,}02$ & $0{,}71\pm0{,}04$ \\
        TP2, votante & $0{,}062\pm0{,}002$ & $0{,}20\pm0{,}02$ & $0{,}64\pm0{,}02$ \\
        \bottomrule
    \end{tabular}
\end{table}

Los dos modelos del TP2 dan tiempos del mismo orden y con la misma pendiente,
como corresponde a una rutina de vecindad idéntica en ambos. Al multiplicar
$N$ por cuatro, el tiempo por llamada crece un factor $9{,}8$ en Vicsek y
$10{,}2$ en el votante, y $11$ en el TP1. El crecimiento supera al de $N$
porque $L$ y $M$ se mantienen fijos: al aumentar $N$ crece la ocupación de
cada celda, y con ella la cantidad de pares que el método revisa por
partícula.

El votante coincide con el TP1 dentro de la dispersión en los tres tamaños
(cocientes $1{,}1$; $1{,}1$ y $1{,}0$). Vicsek queda un $20\%$ por encima en
$N=200$ y $400$ (cocientes $1{,}2$; $1{,}3$ y $1{,}1$). Como el código del
CIM es el mismo y el cronómetro envuelve únicamente la llamada a
\texttt{findNeighbors}, la diferencia está en la configuración sobre la que
trabaja el método: con $\eta=0{,}5$ rad Vicsek forma bandadas, las celdas
quedan desbalanceadas y el CIM revisa más pares por partícula, mientras que a
ese ruido el votante casi no se ordena y su configuración es uniforme como la
del bench. Se lo verificó repitiendo las corridas de Vicsek con
$\eta=6{,}28$ rad, donde el sistema queda desordenado: el tiempo baja de
$0{,}063$ a $0{,}045$ ms en $N=200$ (factor $1{,}4$) y de $0{,}68$ a
$0{,}58$ ms en $N=800$ (factor $1{,}2$). Otras dos causas posibles se
descartaron con la misma prueba: extender la corrida a 20000 pasos (JIT) o
suprimir la escritura del estado en cada paso (entrada/salida) no cambia el
tiempo en $N=200$ ($0{,}069$ y $0{,}065$ ms contra $0{,}063$).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\figw]{cim_tp1_vs_tp2.png}
    \caption{Tiempo por llamada al CIM en función de la cantidad de partículas,
    para el bench del TP1 y para ambos modelos del TP2,
    sobre la geometría común detallada en el texto.}
    \label{fig:cim}
\end{figure}
...
mayores a medida que aumentó la densidad: en $\eta=3$ rad la polarización fue
$0{,}39\pm0{,}04$; $0{,}54\pm0{,}01$ y $0{,}614\pm0{,}003$ para $\rho=2$,
$4$ y $8$. Para $\rho=4$ pasó de $1{,}000$ sin ruido a $0{,}783\pm0{,}007$
en $\eta=2$ rad y a $0{,}0443\pm0{,}0008$ en $\eta=6{,}28$ rad.

El votante fue más sensible al ruido y la densidad no lo protegió. Para
$\rho=4$ y $\eta=0{,}25$ rad, su polarización fue $0{,}53\pm0{,}08$,
mientras que Vicsek conservó $0{,}9966\pm0{,}0002$; al pasar de $\rho=2$ a
$8$ la polarización del votante en ese ruido bajó de $0{,}6\pm0{,}1$ a
$0{,}39\pm0{,}09$.
...
cuando la polarización alcanzó su nivel residual. Su único rasgo es un mínimo
en la transición de cada modelo: $0{,}96$ en $\eta=3$ a $3{,}5$ rad para
Vicsek y $0{,}98$ en $\eta=0{,}5$ rad para el votante, con $\rho=2$. Una
...
conectividad. Estos datos no permiten asignar una relación causal entre ambos
observables.

El CIM dentro de la simulación coincide con el bench del TP1 dentro de la
dispersión para el votante y queda un $20\%$ por encima para Vicsek en
$N=200$ y $400$, por el desbalance de celdas que producen las bandadas.
