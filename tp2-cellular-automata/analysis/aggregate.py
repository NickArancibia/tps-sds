"""Reduce el barrido completo (2 modelos × 6 ρ × 15 η × 50 seeds) a `out/summary.csv`.

Para cada corrida se calcula el promedio temporal de v_a y S en el estado estacionario
(última mitad de la corrida, t >= T/2); para cada punto (model, ρ, η) se reporta el
promedio sobre seeds y su desvío estándar muestral (ddof=1), que es la barra de error de
las curvas η vs observable.
...
from common import (MODELS, RHOS, SUMMARY_CSV, TRANSIENT_FRACTION, load_observables,
...
def main() -> None:
    ...
    with Pool() as pool:
        for model, rho, eta in points:
            ...
            rows.append({
                "model": model, "rho": float(rho), "eta": float(eta), "seeds": len(run_dirs),
